...
func buildPluginsInstallCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "install",
		Short: "Restore all plugins from plugins-lock.json",
		Long: `Restore every plugin recorded in plugins-lock.json, re-fetching each
