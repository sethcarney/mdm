...
Synced skills are recorded in `mdm.lock` with `sourceType: "local"` and a relative path to the `node_modules` directory. Running `mdm skills sync` again after an `npm install` update will re-sync with the new version.
