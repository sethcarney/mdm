...
| any other frontmatter key | omitted when converting markdown for Codex |

When installing a markdown definition to Codex, mdm writes only `name`,
`description`, and `developer_instructions` (from the markdown body).
Other frontmatter keys, including `model-tier`, `model`, `tools`, and nested
configuration, are omitted. Even a frontmatter key named
`developer_instructions` is omitted: the body supplies the instructions.
The source, canonical markdown file, and installs to markdown harnesses keep
their original metadata.

To supply additional Codex configuration, author a native TOML definition.
Codex receives that file unchanged. TOML-to-markdown conversion preserves
additional keys as YAML frontmatter, including nested maps such as
`mcp_servers` and `skills.config`; comments and key order do not survive
conversion. Converting that markdown back for Codex emits only the three
core fields.

Reinstalling or updating a definition regenerates its Codex file with this
restriction. mdm also recognizes unedited files generated by the older
converter, which passed through all frontmatter keys.
